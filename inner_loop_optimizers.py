...
class GradientDescentLearningRule(nn.Module):
    ...
    def update_params(self, names_weights_dict, names_grads_wrt_params_dict, num_step, lr=None, tau=0.9):
        ...
        for i, key in enumerate(names_weights_dict.keys()):
            if lr is None:
                learning_rate = self.learning_rate
                updated_names_weights_dict[key] = names_weights_dict[key] - learning_rate * \
                ...
                updated_names_weights_dict[key] = names_weights_dict[key] - learning_rate * \
                    names_grads_wrt_params_dict[key]
            self.updated_norm.append((learning_rate * names_grads_wrt_params_dict[key]).norm().item())
...
class LSLRGradientDescentLearningRuleALFA(nn.Module):
    """Simple (stochastic) gradient descent learning rule.
    For a scalar error function `E(p[0], p_[1] ... )` of some set of
    potentially multidimensional parameters this attempts to find a local
    minimum of the loss function by applying updates to each parameter of the
    ...
    def __init__(self, device, total_num_inner_loop_steps, use_learnable_learning_rates, use_learnable_weight_decay, alfa, random_init, init_learning_rate=1e-3, init_weight_decay=5e-4):
        ...
        Args:
            init_learning_rate: A postive scalar to scale gradient updates to the
                ...
                if set too small learning will proceed very slowly.
        """
        super(LSLRGradientDescentLearningRuleALFA, self).__init__()
...
class LSLRGradientDescentLearningRule(nn.Module):
    ...
    def update_params(self, names_weights_dict, names_grads_wrt_params_dict, num_step, lr=None, tau=0.1):
        """Applies a single gradient descent update to all parameters.
        All parameter updates are performed using in-place operations and so
        nothing is returned.
        Args:
        ...
        for key in names_grads_wrt_params_dict.keys():
            updated_names_weights_dict[key] = names_weights_dict[key] - \
                                              ...
                                              * names_grads_wrt_params_dict[key]
            self.updated_norm.append((self.names_learning_rates_dict[key.replace(".", "-")][num_step] * \
                names_grads_wrt_params_dict[key]).norm().item())

        return updated_names_weights_dict

        
class LSLRGradientDescentLearningRuleBOIL(nn.Module):
    """Simple (stochastic) gradient descent learning rule.
    For a scalar error function `E(p[0], p_[1] ... )` of some set of
    potentially multidimensional parameters this attempts to find a local
    minimum of the loss function by applying updates to each parameter of the
    ...
    def __init__(self, device, total_num_inner_loop_steps, use_learnable_learning_rates, init_learning_rate=1e-3):
        ...
        Args:
            init_learning_rate: A postive scalar to scale gradient updates to the
                ...
                if set too small learning will proceed very slowly.
        """
        super(LSLRGradientDescentLearningRuleBOIL, self).__init__()
        print(init_learning_rate)
        assert init_learning_rate > 0., 'learning_rate should be positive.'
    ...
    def initialise(self, names_weights_dict):
        self.names_learning_rates_dict = nn.ParameterDict()
        num_layers = len(names_weights_dict)
        for idx, (key, param) in enumerate(names_weights_dict.items()):
            if idx < num_layers -2:
                self.names_learning_rates_dict[key.replace(".", "-")] = nn.Parameter(
                ...
                self.names_learning_rates_dict[key.replace(".", "-")] = nn.Parameter(
                    data=torch.zeros(self.total_num_inner_loop_steps) * self.init_learning_rate,
                    requires_grad=self.use_learnable_learning_rates)
