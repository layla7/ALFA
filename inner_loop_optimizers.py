...
class LSLRGradientDescentLearningRuleRM3(nn.Module):
    """Simple (stochastic) gradient descent learning rule.
    For a scalar error function `E(p[0], p_[1] ... )` of some set of
    potentially multidimensional parameters this attempts to find a local
    minimum of the loss function by applying updates to each parameter of the
    ...
    def __init__(self, device, total_num_inner_loop_steps, use_learnable_learning_rates, init_learning_rate=1e-3):
        ...
        Args:
            init_learning_rate: A postive scalar to scale gradient updates to the
                ...
                if set too small learning will proceed very slowly.
        """
        super(LSLRGradientDescentLearningRuleRM3, self).__init__()
        print(init_learning_rate)
        assert init_learning_rate > 0., 'learning_rate should be positive.'
        ...
        self.gradients_buffer = [None for _ in range(self.ringbuffer_size)]
    ...
    def update_params(self, names_weights_dict, names_grads_wrt_params_dict, num_step, tau=0.1):
        ...
        Args:
            grads_wrt_params: A list of gradients of the scalar loss function
                ...
                previously, with this list expected to be in the same order.
        """
        self.gradients_buffer[num_step % 3] = names_grads_wrt_params_dict

        if num_step >= self.ringbuffer_size - 1:
            sum_gradients = functools.reduce(add_dicts, self.gradients_buffer)
            min_gradients = functools.reduce(min_dicts, self.gradients_buffer)
            max_gradients = functools.reduce(max_dicts, self.gradients_buffer)
            names_grads_wrt_params_dict = functools.reduce(sub_dicts, [sum_gradients, min_gradients, max_gradients])
...
def min_dicts(dict1, dict2):
    out = {}
    for key in dict1.keys():
        out[key] = torch.min(dict1[key], dict2[key])
    return out


def max_dicts(dict1, dict2):
    out = {}
    for key in dict1.keys():
        out[key] = torch.max(dict1[key], dict2[key])
    return out